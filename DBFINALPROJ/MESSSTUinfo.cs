...
{
    public partial class MESSSTUinfo : Form
    {
        ...
        private void MESSSTUinfo_Load(object sender, EventArgs e)
        {
        ...
        private void updateGrid()
        {
            ...
            cmd1 = new OracleCommand("SELECT DOB,ATT FROM MESS_ATTENDENCE WHERE BATCH=:bax AND ROLLNO=:rn", con);
            ...
            {
                cmd = new OracleCommand("SELECT SDNT_INFO.NAME, SDNT_INFO.BATCH, SDNT_INFO.ROLLNO, SDNT_INFO.GENDER, SDNT_INFO.CONTACT_NO, MESS_ATTENDENCE.DOB, MESS_ATTENDENCE.ATT FROM SDNT_INFO INNER JOIN MESS_ATTENDENCE ON SDNT_INFO.BATCH = MESS_ATTENDENCE.BATCH AND SDNT_INFO.ROLLNO = MESS_ATTENDENCE.ROLLNO AND SDNT_INFO.BATCH=:bax AND SDNT_INFO.ROLLNO=:rn", con);
                cmd.Parameters.Add(new OracleParameter("bax", SetValueForText3));
                cmd.Parameters.Add(new OracleParameter("rn", SetValueForText4));
