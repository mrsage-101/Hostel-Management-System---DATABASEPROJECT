...
{
    public partial class FormPersonalSTU : Form
    {
        ...
        private void FormPersonalSTU_Load(object sender, EventArgs e)
        {
        ...
        private void updateGrid()
        {
            ...
            cmd.Parameters.Add(new OracleParameter("pass", SetValueForText5));
            
            OracleDataReader r = cmd.ExecuteReader();
            string[] feilds = new string[3];

            if (r.Read() && r.GetValue(0).ToString() == SetValueForText3 && r.GetValue(1).ToString() == SetValueForText4 && r.GetValue(2).ToString() == SetValueForText5)
            {
                cmd = new OracleCommand("SELECT * FROM SDNT_INFO WHERE BATCH=:bax AND ROLLNO=:rn AND PASSWORD=:pass", con);
                cmd.Parameters.Add(new OracleParameter("bax", SetValueForText3));
                cmd.Parameters.Add(new OracleParameter("rn", SetValueForText4));
                cmd.Parameters.Add(new OracleParameter("pass", SetValueForText5));
        ...
        {
            this.Hide();
            HostelSTUinfo HSI = new HostelSTUinfo();
            HSI.ShowDialog();
        }
        ...
        {
            this.Hide();
            MESSSTUinfo MSI = new MESSSTUinfo();
            MSI.ShowDialog();
        }
        ...
        private void textGAURDIAN_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (!char.IsControl(e.KeyChar) && !char.IsLetter(e.KeyChar))
            {
                e.Handled = true;
